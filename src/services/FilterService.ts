        ...
        return this.getOperators()
            ...
            .map((operator) => {
                ...
                switch (operator) {
                    ...
                    //
                    default: label = Str.human(label); break;
                }
        ...
        const columns = [
            ...attributes.filter((attribute) => {
                ...
                .map((attribute) => {
                    ...
                    return {
                        key: attribute.name,
                        label: Str.human(attribute.name),
                        type,
    ...
    checkIfCanApplyFilters(columnsFilter: FilteredColumn[]): boolean
    {
        ...
        const validatedSelectors = columnsFilter.map((column) => {
            ...
            if ([ 'boolean' ].includes(this.getInputType(column.type))) {
                if (typeof column.value === 'undefined' || Obj.isNull(column.value)) {
                    return true;
                }
...
export default Reducible(FilterService);